...
flowchart LR
 flowchart LR
    A[Iniciar] --> B{Precisa de ambiente isolado?}
    B -->|Sim| C[Instalar Conda]
    B -->|Não| D{Precisa de contêineres portáteis?}
    D -->|Sim| E[Instalar Docker]
    D -->|Não| F[Instalar PIP]
    
    C --> G[Finalizar]
    E --> G
    F --> G

    classDef default fill:#d3d3d3,stroke:#999,stroke-width:2px;
